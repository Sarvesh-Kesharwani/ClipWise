import type { AppData } from '../types';

const GOOGLE_IDENTITY_SCRIPT = 'https://accounts.google.com/gsi/client';
const DRIVE_API_BASE = 'https://www.googleapis.com/drive/v3';
const DRIVE_APPDATA_SCOPE = 'https://www.googleapis.com/auth/drive.appdata';
const SYNC_FILE_NAME = 'clipwise-data.json';
const BACKUP_FOLDER_NAME = 'clipwise-backups';

export interface LegacyDrivePayload {
  app: 'clipwise';
  schemaVersion: number;
  savedAt: number;
  data: AppData;
}

interface DriveFileRef {
  id: string;
  name: string;
  modifiedTime?: string;
}

interface TokenResponse {
  access_token?: string;
  error?: string;
  error_description?: string;
}

interface TokenClient {
  requestAccessToken: (overrideConfig?: { prompt?: string }) => void;
}

interface TokenClientConfig {
  client_id: string;
  scope: string;
  prompt?: string;
  callback: (response: TokenResponse) => void;
  error_callback?: (error: unknown) => void;
}

declare global {
  interface Window {
    google?: {
      accounts: {
        oauth2: {
          initTokenClient: (config: TokenClientConfig) => TokenClient;
        };
      };
    };
  }
}

export function getGoogleClientId(): string {
  return import.meta.env.VITE_GOOGLE_CLIENT_ID ?? '';
}

export async function requestLegacyDriveToken(clientId: string): Promise<string> {
  if (!clientId) {
    throw new Error('VITE_GOOGLE_CLIENT_ID is not configured.');
  }

  await loadGoogleIdentityScript();

  return new Promise((resolve, reject) => {
    const tokenClient = window.google?.accounts.oauth2.initTokenClient({
      client_id: clientId,
      scope: DRIVE_APPDATA_SCOPE,
      prompt: 'consent',
      callback: (response) => {
        if (response.error) {
          reject(new Error(response.error_description || response.error));
          return;
        }
        if (!response.access_token) {
          reject(new Error('Google did not return an access token.'));
          return;
        }
        resolve(response.access_token);
      },
      error_callback: reject,
    });

    tokenClient?.requestAccessToken({ prompt: 'consent' });
  });
}

export async function loadBestLegacyDrivePayload(accessToken: string): Promise<LegacyDrivePayload | null> {
  const candidates: LegacyDrivePayload[] = [];
  const primary = await findSyncFile(accessToken);
  if (primary) {
    const payload = await downloadPayload(accessToken, primary.id);
    if (payload) candidates.push(payload);
  }

  const backupFolder = await findBackupFolder(accessToken);
  if (backupFolder) {
    const backups = await listBackupFiles(accessToken, backupFolder.id);
    for (const backup of backups.slice(0, 20)) {
      const payload = await downloadPayload(accessToken, backup.id);
      if (payload) candidates.push(payload);
    }
  }

  return candidates
    .filter(payload => hasUserData(payload.data))
    .sort((a, b) => b.savedAt - a.savedAt)[0] ?? null;
}

function hasUserData(data: AppData) {
  return (Array.isArray(data.videos) && data.videos.length > 0)
    || (Array.isArray(data.instances) && data.instances.length > 0)
    || (Array.isArray(data.remixes) && data.remixes.length > 0)
    || (Array.isArray(data.feedLists) && data.feedLists.length > 0)
    || (Array.isArray(data.featureRequests) && data.featureRequests.length > 0);
}

async function findSyncFile(accessToken: string): Promise<DriveFileRef | null> {
  const params = new URLSearchParams({
    spaces: 'appDataFolder',
    fields: 'files(id,name,modifiedTime)',
    pageSize: '10',
    q: `name = '${SYNC_FILE_NAME}' and trashed = false`,
  });
  const response = await driveFetch(accessToken, `${DRIVE_API_BASE}/files?${params.toString()}`);
  const body = await response.json() as { files?: DriveFileRef[] };
  return body.files?.[0] ?? null;
}

async function findBackupFolder(accessToken: string): Promise<DriveFileRef | null> {
  const params = new URLSearchParams({
    spaces: 'appDataFolder',
    fields: 'files(id,name,modifiedTime)',
    pageSize: '5',
    q: `name = '${BACKUP_FOLDER_NAME}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false`,
  });
  const response = await driveFetch(accessToken, `${DRIVE_API_BASE}/files?${params.toString()}`);
  const body = await response.json() as { files?: DriveFileRef[] };
  return body.files?.[0] ?? null;
}

async function listBackupFiles(accessToken: string, folderId: string): Promise<DriveFileRef[]> {
  const params = new URLSearchParams({
    spaces: 'appDataFolder',
    fields: 'files(id,name,modifiedTime)',
    pageSize: '100',
    q: `'${folderId}' in parents and trashed = false`,
  });
  const response = await driveFetch(accessToken, `${DRIVE_API_BASE}/files?${params.toString()}`);
  const body = await response.json() as { files?: DriveFileRef[] };
  return (body.files ?? [])
    .filter(file => file.name.endsWith('.json'))
    .sort((a, b) => (b.modifiedTime ?? '').localeCompare(a.modifiedTime ?? ''));
}

async function downloadPayload(accessToken: string, fileId: string): Promise<LegacyDrivePayload | null> {
  const response = await driveFetch(accessToken, `${DRIVE_API_BASE}/files/${fileId}?alt=media`);
  const payload = await response.json() as unknown;
  return isLegacyDrivePayload(payload) ? payload : null;
}

function isLegacyDrivePayload(value: unknown): value is LegacyDrivePayload {
  if (!value || typeof value !== 'object') return false;
  const payload = value as Partial<LegacyDrivePayload>;
  return payload.app === 'clipwise'
    && typeof payload.schemaVersion === 'number'
    && typeof payload.savedAt === 'number'
    && Boolean(payload.data)
    && Array.isArray(payload.data?.videos)
    && Array.isArray(payload.data?.instances);
}

async function loadGoogleIdentityScript(): Promise<void> {
  if (window.google?.accounts.oauth2) return;

  await new Promise<void>((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>(`script[src="${GOOGLE_IDENTITY_SCRIPT}"]`);
    if (existing) {
      existing.addEventListener('load', () => resolve(), { once: true });
      existing.addEventListener('error', () => reject(new Error('Failed to load Google sign-in.')), { once: true });
      return;
    }

    const script = document.createElement('script');
    script.src = GOOGLE_IDENTITY_SCRIPT;
    script.async = true;
    script.defer = true;
    script.onload = () => resolve();
    script.onerror = () => reject(new Error('Failed to load Google sign-in.'));
    document.head.appendChild(script);
  });

  if (!window.google?.accounts.oauth2) {
    throw new Error('Google sign-in was not available after loading.');
  }
}

async function driveFetch(accessToken: string, input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
  const headers = new Headers(init.headers);
  headers.set('Authorization', `Bearer ${accessToken}`);

  const response = await fetch(input, { ...init, headers });
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(errorText || `Google Drive request failed with ${response.status}.`);
  }
  return response;
}
